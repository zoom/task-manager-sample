 ...
 */
function rand (fmt: BufferEncoding, depth = 32) {
 ...
 */
function apiRequest(method: string, endpoint: string, token: string, data = null) {
    return axios({
        data,
        method,
 ...
 * @return {Promise}
 */
export function getDeeplink(token: string | null | undefined) {
    if (!token) {
        token = "";
    }
    return apiRequest('POST', '/zoomapp/deeplink', token, {})
        .then((data) => Promise.resolve(data.deeplink)).catch((e) => console.log(e.response));
}